...
#
# Development environment for casket-ssg.
#
...
# (just haskell) — not a generic estate default.
#
#   nix develop      # enter the shell
#   nix flake check  # verify this file evaluates (run before committing)
{
  description = "casket-ssg development environment";
    ...
    in
    {
      devShells = forAllSystems (pkgs: {
        default = pkgs.mkShell {
          packages = with pkgs; [ just ghc cabal-install ];
        };
      });
    };
}
