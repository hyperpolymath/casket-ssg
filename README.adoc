...
Tagged releases provide a checksummed, provenance-attested core binary for
Linux x86-64. The core build has no LoL native-library requirement. Download
the archive and its adjacent `+.sha256+` file from the
https://github.com/hyperpolymath/casket-ssg/releases[GitHub Releases page],
then verify both its checksum and GitHub build provenance before extracting it:

[source,bash]
----
sha256sum -c casket-ssg-1.0.0-x86_64-linux.tar.gz.sha256
gh attestation verify casket-ssg-1.0.0-x86_64-linux.tar.gz \
  --repo hyperpolymath/casket-ssg
----

casket is not yet on Hackage. To build from source (GHC 9.4+, Cabal 3 or
Stack 2):
...
The experimental https://github.com/hyperpolymath/lol[LoL] native bridge is
an explicit compile-time option:

[source,bash]
----
cabal build -flol-i18n --extra-lib-dirs=/path/to/liblol/lib
----

That option requires a compatible `+liblol+` (currently Zig 0.15.2) and
compiles the Haskell FFI bridge. It does not yet activate translation or
locale-aware rendering in the casket CLI; that product integration remains
planned. Keeping it opt-in prevents an unused native dependency from blocking
the core site generator while preserving a clear integration boundary.
...
* LoL-backed translation and locale-aware rendering through the optional
`+lol-i18n+` native bridge
...
off it)
* Optional LoL bridge: a compatible `+liblol+`; not required by core builds
